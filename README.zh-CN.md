...
**正文里的图片**：agent 写的 `![图说](/path/to/x.png)` 会被上传到飞书，手机上直接看得到图
（需要应用有 `im:resource:upload` 权限）。传不了就退化成一行「🖼 图说 + 路径」，并把原因
写上（文件不存在 / 超 10MB / 缺权限）。这不只是好看：飞书把 `![]()` 当图片语法解析，括号里
必须是它自己的 image_key，留着本机路径会让**整张卡被拒收**——踩过的坑是「有摘要卡但没有
完成卡」，而完成卡是远程继续对话的唯一入口。单张卡最多传 5 张图（hook 是阻塞的，不能为
一屏图让人干等），相对路径按工作区根解析。
...
- `im:chat:readonly`
- `im:resource:upload`（可选）— agent 正文里的本机图片会被传到飞书，手机上直接看得到图。
  不开也能用：图片会退化成一行「🖼 图说 + 路径」的文字，卡片照常发出。
