...
- **客户端有两道时限，而且必须发心跳才能活过第一道。** 这是 2026-09-01 踩出来的：GY_2 上
  每一张提问卡都只等了 **120 秒**就失效，卡面上写的 12 小时纯属空话。Cursor 自己的结构化
  日志（`mcpMeta.event=mcp_tool_call_timeout`）把常量写得很清楚：

  | 常量 | 值 | 含义 |
  | --- | --- | --- |
  | `idleTimeoutMs` | 120000 | 连接上 120 秒没动静，本次 `tools/call` 判死（`MCP error -32001`），与总时长无关 |
  | `maxTotalTimeoutMs` | 3600000 | 单次调用的硬顶，心跳也顶不过去 |

  `timeoutKind: idle` 这个字段是关键线索：它说明 Cursor 用的是 MCP SDK 的
  `resetTimeoutOnProgress` 语义 —— **服务端周期发 `notifications/progress`，idle 计时器
  就会被重置**。实测（`/tmp` 探针，见下）：不发心跳一律死在第 120 秒；每 20 秒发一次，
  单次调用活到 300 秒仍正常返回，且客户端没发 `notifications/cancelled`。
  所以 `CURSOR_ASK_HEARTBEAT_SEC`（默认 20 秒）不是「进度展示」，是**保命信号**。

  别再相信「`notifications/progress` 不会续期」那句话（它来自论坛里 CLI/ACP 那条路的
  报告，IDE 这条路不成立，这个错误前提曾经写在本文件和代码注释里）。

- **心跳只解决 idle，解决不了 60 分钟硬顶，所以仍然要分段续等。** 总窗口
  `CURSOR_ASK_TIMEOUT_SEC`（默认 24 小时，与 `CURSOR_FOLLOWUP_TIMEOUT_SEC` 对齐），单段
  `CURSOR_ASK_CHUNK_SEC`（默认 50 分钟，代码里会夹到 55 分钟以内）；每段到点若还没答案
  就返回 `pending_id`，要求 agent 立刻调
  `ask_user_wait` 继续等。客户端万一没给 `_meta.progressToken`（心跳无处可发），单段自动
  退化成 90 秒，抢在 idle 判死前把控制权交还 agent —— 慢，但不会静默失联。
- **`notifications/cancelled` 必须处理。** 客户端放弃后我们若还在傻等，用户此刻在飞书上
  的回答会被写进一个**没人收**的返回值里：卡片显示「已回答」，agent 什么都没收到。
  收到取消就当场收手，并把卡片收敛成「已取消 / 你的回答到得太晚」，如实告诉用户。
...
**`mcp.json` 也是热加载的（2026-09-01 更正，此前本文件写的「必须重开窗口」是错的）**：
往里加一个服务，6 秒内新进程就起来了，同一个会话里当场就能调它的工具；把条目删掉，
那个进程会被自己回收。但要注意区分**配置**和**代码**：

- 改了某个服务在 `mcp.json` 里的条目 → Cursor 重启**该**服务；
- 只改服务端代码（`.js` 文件）→ **不会**重启，进程还是老的，仍跑着旧代码。

所以远程机上 rsync 完 MCP 服务的代码，必须让它的进程换掉才生效：**给它的条目加一个无害的
`env` 键**（实测这一步稳定触发重启；反过来「把刚加的键删掉」不一定触发，别指望它），
或者重开窗口。也别指望 `kill` 掉进程能自愈——实测杀掉后不会自动重生，得改配置或重开窗口。
这一点和 hooks 完全不同，hooks 是每次事件重新 spawn 的，天然拿到新代码。
...
- **工具失败没有独立卡片**：`postToolUseFailure` 与 `postToolUse` 同归 `kind=live`，失败只是
  摘要卡里的一步（标题写 ❌ + 失败原因，error_message 占「结果」的位置，卡头挂红色失败计数）。
  每个失败工具发一张红卡太吵；但也别反过来把这个事件丢掉——那样失败的步骤会从摘要里凭空消失，
  读摘要的人只看到成功的部分。用户主动中断（`is_interrupt`）不算失败，不进摘要。
- **失败那一步也默认折叠**，别给它开 `expanded`：报错正文动辄几十行，自动展开会把整张摘要
  撑长，反而更难读。要显眼就靠标题和卡头的红标签。
- **助手正文只落一处，默认落在完成卡上**（`handleResponse`）。`afterAgentResponse` 每轮
  **只在轮末触发一次**——GY_2 上 9 轮真实条目的形状清一色是 `ttttT`，连 27 次工具调用的长
  轮次里也只有结尾那一条正文。所以摘要卡里那段正文【永远】就是紧随其后那张完成卡的正文，
  两张卡前后脚发出，同一段话读两遍。只有 `CURSOR_NOTIFY_STOP=0`（压根不发完成卡）时才让
  摘要卡兜住它：`notifyStop` 为真时 `handleFollowup` 必定发卡（waiting 是交互卡，否则是
  纯通知卡），两者都带 body。这也顺带解决了「纯回话轮次」——那种轮次只有一条正文、没有
  工具，摘要卡整张就是完成卡的副本；不收正文之后它连卡都不会发（`hasContent` 判据）。
- **助手正文面板恒定默认展开**（`assistantPanel()`，CLI 轮次卡与上面那种兜底场景共用）。
...
Cursor 的 `output` 有个例外：它只在 `CURSOR_NOTIFY_STOP=0` 时才真的把助手正文放进摘要卡，
否则那段话由完成卡承载 —— Cursor 的 `afterAgentResponse` 只在轮末触发，两处都放就是重复
（见 §4「Cursor 卡片的特殊约束」）。
